    ...
    ) : BackgroundService
{
    ...
    private const string TransactionResultExchange = "transaction.request";
    private IConnection? _connection;
    private IChannel? _channel;
    private AsyncEventingBasicConsumer? _consumer;
    private const string QueueName = "NotificationServiceTransactionResultConsumer";
    ...
    private async Task HandleMessageAsync(object sender, BasicDeliverEventArgs ea)
    {
        try
        {
            ...
            if (result != null)
            {
                ...
                {
                    var notificationService = scope.ServiceProvider.GetRequiredService<NotificationManager>();
                        if (result != null)
                        {
                            await notificationService.SendMessageAsync(result);
                        }
                        else
                        {
                            logger.LogError($"Transaction error: no result");
                        }
